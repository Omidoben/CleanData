...
clean_unemp <- function(unemp_data,
  ...
  if(is.null(year_of_interest)) {
    year_of_interest <- unique(unemp_data$year)
  }
  if(is.null(place_name_of_interest)) {
    place_name_of_interest <- unique(unemp_data$place_name)
  }
  if(is.null(level_of_interest)) {
    level_of_interest <- unique(unemp_data$level)
  }

  result <- unemp_data |>
    janitor::clean_names() |>
    dplyr::filter(
      year %in% {{year_of_interest}},
